...
(function($) {
	...
	var handlers = {
		...
		onDialogOpen: function() {
			...
			this.queueData.errorMsg = '文件未添加至队列:';
		...
		onSelect: function(file) {
			...
			for (var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					...
					var replaceQueueItem = confirm('文件 "' + file.name + '" 已经添加至上传队列.\n是否需要替换?');
					if (!replaceQueueItem) {
				...
				switch (errorCode) {
					case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
						if (settings.queueSizeLimit > errorMsg) {
							...
							this.queueData.errorMsg += "每次最多上传 " + settings.queueSizeLimit + "个文件";
						}
						break;
					...
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						...
						this.queueData.errorMsg += "文件格式不正确，仅限 " + settings.fileTypeExts;
						break;
				}
			}
				...
				$.ajax({
					...
					success: function(data) {
						if (data == 1) {
							...
							var overwrite = confirm('文件 "' + file.name + '" 在服务中已经存在.\n是否需要替换?');
							if (!overwrite) {
								this.cancelUpload(file.id);
								$('#' + file.id).remove();
